...
<decision>
 <category>UI_UX</category>
 <context>The Rx desk shipped with five advice lines and nine history codes hardcoded in `AdviceChips` and the desk blade. A doctor's actual advice is his own — a dermatologist and a cardiologist do not tell patients the same things — and the only way to keep a line was the ★, which wrote to that browser's `localStorage`: the same doctor had one set at the chamber desk and another on his laptop, and could edit neither.</context>
 <action>Advice and History chips are now curated on **My medicines**, beside the medicine list, backed by a `doctor_chips` table read through `DoctorChipService`. The shipped chips stay in code with stable keys; the table holds only departures from them (a row with `default_key` overrides or hides a built-in, a row without one is a chip the doctor added). Advice carries a doctor-facing label and an optional Bangla line for the patient, falling back to the label. History carries a label and `is_primary` (first row vs behind "More…"). The ★ on the desk now writes a chip through the same service instead of `localStorage`.</action>
 <reason>Same principle already settled for medicines and packs (2026-08-11, 2026-08-12): a doctor's shortcuts are a list he can see and edit, never something the app infers or hides in a browser. Keeping the defaults in code rather than seeding rows means a new doctor is useful on day one, translations still ship with a deploy, and a chamber that never touches the page carries no rows at all.</reason>
</decision>
