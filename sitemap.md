# Site Map
Last Updated: 2026-08-19T21:43:57+0600
...
- **Data/systems touched:** `platform_settings.patient_booking_horizon_days`. Public book APIs and the clinic hero **Select date** list (sitting days only) read it; desk walk-ins do not.
...
- **Steps:** Super Admin turns on **Referrals** for the tenant. Owner adds GPs under **Operations → Referring doctors**. At **Collect fee** or **New Walk-In**, desk staff pick **Referred by (outside GP)**. When the fee is collected, the system owes ৳200 (visit), ৳1,000 (intervention), or the MSK cut once `referral_msk_commission_taka` is set (default ৳0, no row). End of month → **Operations → Referral ledger** → select pending rows → **Mark selected as paid** (posts one cashbook payout). Missed visits after door pay void pending commissions.
