...
| `/{slug}/admin/pharmacy-counter` | Pharmacy till: sell from Rx or walk-in, medicine voucher print, same-day return | staff with money job / admin (**Pharmacy**) |
| `/{slug}/pharmacy-sales/{sale}/invoice` | A4 medicine slip using the prescription pad (auto-print) | staff with money job / admin (**Pharmacy**) |
| `/{slug}/fee-receipts/{entry}` | A4 consultation-fee slip using the prescription pad (auto-print) | staff with money job / doctor / admin |
| `/{slug}/admin/pharmacy-items` | Shop stock: add SKU, receive box | staff with money job / admin (**Pharmacy**) |
| `/{slug}/admin/pharmacy-physical-count` | Physical count vs system qty (hidden for now; code kept) | none until re-enabled |
| `/{slug}/admin/pharmacy-pay-supplier` | Pay the company / supplier refund (hidden for now; code kept) | none until re-enabled |
...
| `/admin/pharmacy-counter` | Pharmacy till: sell from Rx or walk-in, medicine voucher print, same-day return | staff with money job / admin (**Pharmacy**) |
| `/pharmacy-sales/{sale}/invoice` | A4 medicine slip using the prescription pad (auto-print) | staff with money job / admin (**Pharmacy**) |
| `/fee-receipts/{entry}` | A4 consultation-fee slip using the prescription pad (auto-print) | staff with money job / doctor / admin |
| `/admin/pharmacy-items` | Shop stock: add SKU, receive box | staff with money job / admin (**Pharmacy**) |
| `/admin/pharmacy-physical-count` | Physical count vs system qty (hidden for now; code kept) | none until re-enabled |
| `/admin/pharmacy-pay-supplier` | Pay the company / supplier refund (hidden for now; code kept) | none until re-enabled |
...
- **Steps:** Staff or owner → **Operations → Book serial** → pick **doctor** (pre-filled when the login only sees one) → pick **visit type** (Usual, Follow-up, Intervention if Stations is on, Lab if Stations or lab tests) → if Intervention and the fee list has procedures, pick **intervention type** (PRP, epidural, …) → if Lab, pick **lab type** (MSK, a named test, or collection window) → pick **centre** only when that doctor sits at more than one place → pick a **date** (visit: greys days they do not sit — not a closed clinic; Lab: any day except vacation) → pick the matching **sitting** → name and phone → optional Different WhatsApp → Book. Confirmation modal shows serial, come-around (or sitting hours), centre, sitting, doctor, phone; Push WhatsApp pastes that same text with a short ticket link / Push SMS when those are on / Open ticket / Done. Auto SMS still goes after the response when ticked. Report / counseling stay on the floor handoff, not this page. **New Walk-In** on Daily Roster / Live Queue is the same visit types for people already at the chamber **today** (overflow stools; Live Queue is already on that sitting).
...
- **Steps:** Super Admin ticks **Pharmacy**. Desk with the **Money** job (or owner) **Pharmacy stock** — add what this shop actually holds (search the national list, set selling price and buying price, pick **Centre** when the clinic has more than one building, receive a box: pay ৳0 / some / full now, mark returnable or bought outright). **MUPS** already has the nine pad SKUs at **each** centre (Coral D Max through Slim Herb); the demo seed receives a small cupboard at both (a handful of bottles each, on credit) so the till can sell without a first Receive. A Mehedibag login does not see Uttara bottles. Same desk opens **Operations → Pharmacy** — pick **Centre** first if this login can sell from more than one building (a Mehedibag-only login skips that; Users → Branches is the stamp). Then pick today’s prescription (that centre’s sittings) or a walk-in (name optional, no phone — unlike Daily Roster walk-in), add lines (medicine list is name, price, stock — not the building name) from this cupboard only (one sale cannot mix two centres), type a **Discount ৳** if the owner knocks some money off (0 = full sticker; the till shows **To collect**), then take cash / bKash / Nagad / card / cash+online for that collected amount (same as Collect fee). The **medicine voucher** opens on the same prescription pad as the doctor’s print (Branding logo left, wider chamber address right, quiet medicine columns — not a shop invoice) and prints Discount when they paid less than list. Live qty drops **at that centre**. **Pay supplier** and **Physical count** are off the menu for now (code kept). Optional **Doctor pharmacy cuts** (Branding % of profit, default 0 = off) only on Rx-linked sales; walk-in is ৳0. Same-day **Return** undoes the sale. A later dedicated chemist staff login can take stock without a new till role — not split yet.
- **Data/systems touched:** `pharmacy_items.chamber_id`, `pharmacy_counts.chamber_id`, `pharmacy_deliveries`, `pharmacy_sales` / `_items` (`receipt_number`), `pharmacy_count_items`, `pharmacy_stock_adjustments`, `pharmacy_supplier_settlements`, `pharmacy_doctor_commissions`, `PharmacySaleService` / `PharmacyInvoiceService` / `TakaWords` / `PharmacyStockService` / `PharmacySupplierService` / `PharmacyDoctorCommissionService` / `StaffDeskScope`, cashbook `pharmacy` / `pharmacy_purchase` / `pharmacy_refund` / `pharmacy_supplier_refund` / `pharmacy_doctor_payout`.
- **Success:** End of day the khata shows the money that actually came in (sticker minus any till discount); the shelf qty matches what was sold/received/counted; the company is owed only for what the deal says (sold vs bought-outright); the doctor is not silently owed a walk-in cut.
...
- **Steps:** Super Admin ticks **Stations** on the tenant. Staff set **Schedule Sessions** for **Visit** and **Intervention** hours (leftover `consult` still resolves). **Branding** ticks which **rooms** exist: Lab, Report, Counseling. A weekday without a doctor sitting means that doctor is not there, not that the centre is closed. **While Live Queue is running** (active / paused / delayed) at that centre, staff can **Send to lab / intervention / report / counseling** from anyone on today's list — any room the clinic has, not only the next step on the usual path. Counseling can share the visit/OT room; another clinic may add Counseling as its own sitting. Missing rooms are skipped. **New visit (when the queue is not running):** from Visit, **Send to lab** (then type — today MSK) and/or **Send to intervention** (then type). Lab and report lists follow Branding rooms, not the doctor’s sitting weekday. Lab still continues to intervention → report → counseling; skipping lab goes straight to intervention then report. **Follow-up** (clinic/doctor rules on Branding / Doctors — default is a completed visit on a previous day within 3 months; after that window a return is a new visit even on the 4th time; unlimited never expires): from Visit, **Send to lab** then **Send to report**, or **Send to intervention** then **Send to counseling**. **Direct intervention** walk-in: Procedure done → **Send to counseling**. **MSK-only (outside GP sent them for a scan, no visit):** New Walk-In with visit type **Lab** on an open clinic day, pick the referring doctor, collect the scan fee at the door. **Daily Roster / Live Queue:** **Outdoor vitals** is a primary on waiting rows (BP + weight) when Stations and the prep desk job are on → doctor consult pad prefills today's numbers. Extra steps sit under **More**. **Send to lab** asks for lab type then puts them on today's list. **Send to intervention** opens a sitting picker plus procedure type from a **visit** (or after lab): same day is listed whenever the clinic is sitting for visits, even if that weekday has no OT hours on the timetable (a same-day OT list is provisioned). Morning OT that already ended stays pickable. The default is the next sitting that has not ended. Confirming creates a linked procedure serial. If they cannot come, **Move intervention** picks another sitting. Forgotten next-day OT dates sit on **Operations → Missed procedures** (WhatsApp + Move; nothing auto-cancels). **Operations → Sitting day overrides** for tomorrow's hours without rewriting Saturday forever. Doctors receive a **Morning queue count** notification after 09:05 when visits or procedures are waiting. Waiting-room TV: per sitting `/screen/{session}`, or **All rooms TV** `/screen/chamber/{chamber}` (up to six live tiles).
...
- **Steps:** Staff (or the account owner; or the doctor only when there is no staff login) → **Settings → Chambers** (address, map link; the list is in the sidebar only when the practice has more than one chamber) and **Settings → Schedule Sessions** (Visit and Intervention hours: which doctor, which day, start/end, how many serials). Lab and report are Branding rooms, not rows on this list.
