...
Last Updated: 2026-08-05T05:56:46+0600
...
- **Web page builder** — Filament WebPages resource; solo template sections under `resources/views/tenant/solo/`; SafeUrl allowlist for links, then `tenant_safe_href()` so relative CTAs (e.g. `/book`) get the path-tenant prefix on central hosts. Solo homepage typography is centralized in `tenant/solo/webpage.blade.php` as `.solo-h1` / `.solo-h2` / `.solo-h3` / `.solo-body-lg` / `.solo-body` / `.solo-tagline` / `.solo-label` / `.solo-question` / `.solo-brand` / `.solo-nav` — section blades must use these classes, not per-file `text-*` sizes. Display headings keep line-height ≥ 1.05 (Figma’s tighter 0.85 clipped live multi-line titles).
