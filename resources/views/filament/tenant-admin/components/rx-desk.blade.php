...
@php
    ...
    $timingOptions = collect(PrescriptionTiming::labels())
        ...
        ->values()
        ->all();
    $historyChips = ['HTN', 'DM', 'Asthma', 'CKD', 'IHD', 'Thyroid', 'Smoker'];
...
<div
    class="cs-rx-desk"
    wire:key="rx-desk-{{ $booking?->id }}-{{ $written?->updated_at?->timestamp ?? 0 }}"
    x-data="rxDesk({
...
>
    <div class="cs-rx-desk__bar">
        ...
        <div class="cs-rx-desk__bar-actions">
            ...
            <x-filament::button type="button" color="gray" size="sm" icon="heroicon-o-bookmark" x-on:click="save()">
                {{ __('Save only') }}
            </x-filament::button>
            @if (auth()->user()?->canOperateQueueControls())
                <x-filament::button type="button" color="success" size="sm" class="cq-needs-network" wire:click="mountAction('completeVisit')">
                    {{ __('Complete visit') }}
                </x-filament::button>
            @endif
        </div>
    ...
    <div class="cs-rx-desk__grid">
        <div class="cs-rx-desk__left">
            ...
            <section class="cs-rx-desk__card">
                ...
                --}}
                <table class="cs-rx-desk__oe-table">
                    <tr>
                        <th>{{ __('Wt') }}</th>
                        <td>
                            <input type="number" step="0.1" min="0.5" max="300" x-model="weightKg" placeholder="kg">
                    ...
                    <tr>
                        <th>{{ __('BP') }}</th>
                        <td>
                            <input type="number" min="60" max="250" x-model="bpSystolic" placeholder="sys" class="cs-rx-desk__bp">
                            <span class="cs-rx-desk__bp-sep">/</span>
                            <input type="number" min="30" max="150" x-model="bpDiastolic" placeholder="dia" class="cs-rx-desk__bp">
                            <span class="cs-rx-desk__unit">mmHg</span>
                            <span
                                class="cs-rx-desk__last-vital"
                                x-show="lastBpSystolic && lastBpDiastolic"
                                x-text="lastBpSystolic + '/' + lastBpDiastolic"
                                x-cloak
                            ></span>
                        </td>
                    </tr>
                    <tr>
                        <th>{{ __('Pulse') }}</th>
                        <td>
                            <input type="number" min="30" max="250" x-model="pulseBpm" placeholder="78">
                            <span class="cs-rx-desk__unit">/min</span>
                    ...
                    <tr>
                        <th>{{ __('SpO₂') }}</th>
                        <td>
                            <input type="number" min="50" max="100" x-model="spo2Percent" placeholder="98">
            ...
            <section class="cs-rx-desk__card">
                ...
                <div class="cs-rx-desk__chips" x-show="suggestedPacks.length || diagnosisAdvice || diagnosisTests" x-cloak>
                    <template x-for="pack in suggestedPacks" :key="pack.id">
                        <button type="button" class="cs-rx-desk__chip cs-rx-desk__chip--strong" x-on:click="applyPack(pack)">
                            <span x-text="pack.name"></span>
                        </button>
                    </template>
                    <button type="button" class="cs-rx-desk__chip" x-show="diagnosisAdvice" x-on:click="applyDiagnosisAdvice()">
                        {{ __('Add advice') }}
                    </button>
        ...
        <div class="cs-rx-desk__right">
            <section class="cs-rx-desk__card cs-rx-desk__rx">
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <tr>
                                    ...
                                    <td class="cs-rx-desk__brand-cell">
                                        ...
                                        {{-- Reason (stored as `indication`): why this medicine, in the
                                             doctor's own words. The phone form has always had it and
                                             print already renders it; the desk had nowhere to type it.
                                             Never pre-filled from the catalogue's drug-class text — that
                                             is a class hint, not this patient's reason. --}}
                                        <input
                                            type="text"
                                    ...
                                    <td>
                                        <input type="text" x-model="item.dose" x-on:focus="focusDoseCell(index)" placeholder="—">
                                    ...
                                    <td>
                                        <input type="text" x-model="item.frequency" x-on:focus="editingCell = 'freq-' + index" placeholder="—">
                                    ...
                                    <td>
                                        <input type="text" x-model="item.duration" x-on:focus="editingCell = 'dur-' + index" placeholder="—">
                ...
                <div class="cs-rx-desk__shorthand">
                    ...
                    >
                    <button type="button" class="cs-rx-desk__link" x-on:click="packMenuOpen = !packMenuOpen">{{ __('Packs') }}</button>
                </div>
                ...
                <div class="cs-rx-desk__packs" x-show="packMenuOpen" x-cloak>
                    <div class="cs-rx-desk__chips" x-show="packs.length">
                        <template x-for="pack in packs" :key="pack.id">
                            <button type="button" class="cs-rx-desk__chip" x-on:click="applyPack(pack)" x-text="pack.name"></button>
                        </template>
                    </div>
            ...
            <section class="cs-rx-desk__card">
                <h3>{{ __('Follow-up') }}</h3>
                <div class="cs-rx-desk__chips">
                    <button type="button" class="cs-rx-desk__chip" :class="{ 'is-on': followUpRelative === '1_week' }" x-on:click="followUpRelative = '1_week'; followUpNote = ''">{{ __('1 week') }}</button>
                    <button type="button" class="cs-rx-desk__chip" :class="{ 'is-on': followUpRelative === '2_weeks' }" x-on:click="followUpRelative = '2_weeks'; followUpNote = ''">{{ __('2 weeks') }}</button>
                    <button type="button" class="cs-rx-desk__chip" :class="{ 'is-on': followUpRelative === '1_month' }" x-on:click="followUpRelative = '1_month'; followUpNote = ''">{{ __('1 month') }}</button>
                    <button type="button" class="cs-rx-desk__chip" :class="{ 'is-on': followUpRelative === 'as_needed' }" x-on:click="followUpRelative = 'as_needed'">{{ __('As needed') }}</button>
                </div>
            </section>

            <section class="cs-rx-desk__card cs-rx-desk__media">
                <button type="button" class="cs-rx-desk__chip" x-on:click="showReports = !showReports">{{ __('Reports') }}</button>
                <button type="button" class="cs-rx-desk__chip" wire:click="mountAction('writePrescription')">{{ __('Voice / photo') }}</button>
                <div x-show="showReports" x-cloak class="cs-rx-desk__reports">
                    <textarea rows="2" x-model="reportsSeen" placeholder="{{ __('Reports the patient brought') }}"></textarea>
                </div>
            </section>
        </div>
    </div>
...
@once
    ...
    <script>
        Alpine.data('rxDesk', (config) => ({
            items: Array.isArray(config.items) ? config.items.map((row) => ({ ...row, timing: row.timing || '' })) : [],
